  ...
  if (parentElems) {
    const updateHeight = (elem) => {
      ...
      if (parentElem) {
        ...
        const variableName = `--${matchedSelector.replace(
          /^([#.])/,
          ""
        )}-height`;
        parentElem.style.setProperty(
          variableName,
          `${elem.getBoundingClientRect().height.toFixed(2)}px`
        );
      }
    };
...
function customSearchClose() {
  const searchButtonClose = document.querySelector(".header__search-close");
  const headerEl = document.querySelector(".header");

  searchButtonClose.onclick = () => {
    headerEl.classList.remove("search-is-visible");
    headerEl.classList.add("search-is-collapsed");
  };
}
